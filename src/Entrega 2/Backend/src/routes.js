...
import { profile, updateMe, deleteUser, createMember } from "./controllers/userController.js";
...
r.post("/groups", async (req, res) => {
  try {
    const { name, members, monetary_target, food_goal } = req.body;
    ...
    const [result] = await pool.execute(
      "INSERT INTO `groups` (name, members, monetary_target, food_goal) VALUES (?, ?, ?, ?)",
      [name.trim(), parseInt(members) || 0, parseFloat(monetary_target) || 0, parseFloat(food_goal) || 0]
    );

    const [rows] = await pool.query("SELECT * FROM `groups` WHERE id = ?", [result.insertId]);
    return res.status(201).json({ ok: true, group: rows[0] });
  } catch (error) {
    console.error("Erro ao criar grupo:", error);
    return res.status(500).json({ error: "Erro interno ao criar grupo.", details: error.message });
  }
});
